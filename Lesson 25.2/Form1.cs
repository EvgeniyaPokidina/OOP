...
namespace Lesson25._2
{
    public partial class Form1 : Form
    {
        ...
        private void UpDateForm(List<Inventory> list)
        {
            ...
            toolStripStatusLabelCount.Text = "Êîëè÷åñòâî çàïèñåé" + list.Count.ToString();
        }
        ...
        private void textBox1_TextChanged(object sender, EventArgs e)
        {
            if (textBoxSearch.Text.Length != 0)
            {
                switch (comboBoxFilter.SelectedIndex)
                {
                    case 0:
                        {
                            List<Inventory> list = records.Where(p => p.Name.ToLower().StartsWith
                            (textBoxSearch.Text)).ToList();
                            UpDateForm(list);    
                        }
                        break;
                    case 1:
                        {
                            List<Inventory> list = records.Where(p => p.Quantity==int.Parse(textBoxSearch.Text)).ToList();
                            UpDateForm(list);
                        }
                        break;
                    case 2:
                        {
                            List<Inventory> list = records.Where(p => p.Price == int.Parse(textBoxSearch.Text)).ToList();
                            UpDateForm(list);
                        }
                        break;
                }
